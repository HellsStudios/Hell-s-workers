...
@onready var btn_close: Button        = %ButtonExit
...
func _cur_fridge_meal_id() -> String:
	...
	if idx < 0:
		idx = 0
	return String(cont_drop.get_item_metadata(idx))
...
func _ready() -> void:
	...
	if btn_close and not btn_close.pressed.is_connected(func(): hide()):
